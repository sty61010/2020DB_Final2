...
//			"CREATE INDEX idx_warehouse ON warehouse (w_id) USING hash",
//			"CREATE INDEX idx_district ON district (d_w_id, d_id) USING hash",
//			"CREATE INDEX idx_customer ON customer (c_w_id, c_d_id, c_id) USING hash",
//			"CREATE INDEX idx_history ON history (h_c_w_id, h_c_d_id, h_c_id, h_id) USING hash",
//			"CREATE INDEX idx_order ON orders (o_w_id, o_d_id, o_id) USING hash",
//			"CREATE INDEX idx_new_order ON new_order (no_w_id, no_d_id, no_o_id) USING hash",
//			"CREATE INDEX idx_order_line ON order_line (ol_w_id, ol_d_id, ol_o_id, ol_number) USING hash",
//			"CREATE INDEX idx_stock ON stock (s_w_id, s_i_id) USING hash",
//			"CREATE INDEX idx_item ON item (i_id) USING hash" };
	
//	private static final String INDEXES_DDL[] = {
//			"CREATE INDEX idx_warehouse ON warehouse (w_id) USING btree",
//			"CREATE INDEX idx_district ON district (d_w_id, d_id) USING btree",
//			"CREATE INDEX idx_customer ON customer (c_w_id, c_d_id, c_id) USING btree",
//			"CREATE INDEX idx_history ON history (h_c_w_id, h_c_d_id, h_c_id, h_id) USING btree",
//			"CREATE INDEX idx_order ON orders (o_w_id, o_d_id, o_id) USING btree",
//			"CREATE INDEX idx_new_order ON new_order (no_w_id, no_d_id, no_o_id) USING btree",
//			"CREATE INDEX idx_order_line ON order_line (ol_w_id, ol_d_id, ol_o_id, ol_number) USING btree",
//			"CREATE INDEX idx_stock ON stock (s_w_id, s_i_id) USING btree",
//			"CREATE INDEX idx_item ON item (i_id) USING btree" };
