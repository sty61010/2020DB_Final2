...
public class TpccTestbedLoaderProc extends AllExecuteProcedure<StoredProcedureParamHelper> {
	...
	protected void executeSql(Map<PrimaryKey, InMemoryRecord> readings) {
		...
		// Generate item records (fully replicated)
		generateItems(1, TpccConstants.NUM_ITEMS);

		// Generate warehouse
		int startWid = Calvin.serverId() * TpccConstants.NUM_WAREHOUSES + 1;
		for (int wcount = 0; wcount < TpccConstants.NUM_WAREHOUSES; wcount++) {
			generateWarehouseInstance(startWid + wcount);
		}
